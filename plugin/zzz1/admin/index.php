...
echo 'Это вывод в админку плагина zzz1';?>
...
if ($post = mso_check_post(array('f_session_id', 'toadd')) or mso_segment(4)=='edit' or $post = mso_check_post(array('f_session_id', 'delladd')))
	...
	$quantity_prod = '0';
	$reserve_prod = '0';
	$shapka=t('Новый товар', 'plugins/grshop');
	$qadd = 0;
	...
	if ($id_prod != '0')
		...
		$shapka = t('Редактирование товарa ID', 'plugins/grshop').': '.$id_prod;
		}

	$arralladd = get_array_db ('add');		// получаем массив с ID и самими add-ами;
	if ($arralladd != FALSE) 
		{
		$arraddf[0] = t('характ.', 'plugins/grshop');
	...
	Echo '<h1 class="content">'.$shapka.'</h1><br />'.
	...
	'<table style="width: 99%; border: none; line-height: 1.4em;"><tr><td style="vertical-align: top; padding: 0 0px 0 0;">
		...
		<td style="vertical-align: top; padding: 0 0px 0 0;">
			...
			<h3>'.t('Название файла изображения', 'plugins/grshop').'</h3>'.form_input('photo_prod', $photo_prod).NR.'
			</div>
		</td>
		<tr>
		<td style="vertical-align: top; padding: 0 0px 0 0;">
			<div class="block_page"><h3>'.t('Складской номер:', 'plugins/grshop').'</h3>'.form_input('id_sklad_prod', $id_sklad_prod).NR.'
			</div>
		</td>
		<td style="vertical-align: top; padding: 0 0px 0 0;">
			<div class="block_page"><h3>'.t('Артикул', 'plugins/grshop').'</h3>'.form_input('articul_prod', $articul_prod).NR.'
			</div>
		</td>
		</tr>
		<tr>
		<td style="vertical-align: top; padding: 0 0px 0 0;">
			<div class="block_page"><h3>'.t('Название товара', 'plugins/grshop').'</h3>'.form_input('name_prod', $name_prod).NR.'
			</div>
		</td>
		<td style="vertical-align: top; padding: 0 0px 0 0;">
			<div class="block_page"><h3>'.t('Базовая цена', 'plugins/grshop').'</h3>'.form_input('cost_prod', $cost_prod).NR.'
			</div>
		</td>
		</tr>
		<tr>
		<td style="vertical-align: top; padding: 0 0px 0 0;">
			<div class="block_page"><h3>'.t('Количество', 'plugins/grshop').'</h3>'.form_input('quantity_prod', $quantity_prod).NR.'
			</div>
		</td>
		<td style="vertical-align: top; padding: 0 0px 0 0;">
			<div class="block_page"><h3>'.t('Резерв', 'plugins/grshop').'</h3>'.form_input('reserve_prod', $reserve_prod).NR.'
			</div>
		</td>
		</tr>
		</table>
	...
	//<div class="block_page"><h3>'.t('Описание', 'plugins/grshop').'</h3>'.form_textarea($data = array('name'=>'description_prod', 'value'=>$description_prod,'rows'=>'5')).NR.'</div>


		
		
	//пробуем вставить человеческий редактор
	require('form.php');	
	$f_content = htmlspecialchars($description_prod);
...
//окончание пробной вставки человеческого редактора	
	...
	if (isset($arraddprodid) && $arraddprodid != FALSE)
		...
		foreach ($arraddprodid as $row)
			...
			$out .= form_input('arr_id_add['.$row['id_add'].']', $row['val_prodadd']).NR.form_submit('delladd['.$row['id_add'].']', t('удалить', 'plugins/grshop') ).'</td></tr>'.NR;
			};		
		}
	
	$out .=  '<tr><td>';
	if (isset($arraddf)) $out .= form_dropdown('id_new_add', $arraddf, '0').' или '.NR;
	else 	{$out .= t('Новая характеристика', 'plugins/grshop').': '.NR;}
	$out .= form_input('name_new_add', '').'</td><td>'.
	form_input('val_new_add', '').NR.form_submit('toadd', t('добавить характеристику', 'plugins/grshop') ).'</td></tr>
	</table>'.
	form_submit('addprod', t('Сохранить', 'plugins/grshop') ).
	'</td><td style="vertical-align: top; width: 250px;">
	<div class="block_page"><h3>'.t('Отображать', 'plugins/grshop').'</h3>'.form_checkbox('public_status_prod', '1', $checket).NR.'</div>
	<p class="info">'.
	t('Отметьте категории, включающие товар', 'plugins/grshop').'
...
if ($post = mso_check_post(array('f_session_id', 'addprod')))
	...
	if ($res != 0) 
		{
		echo '<div class="update">'.t('изменения сохранены', 'plugins/grshop').'</div>';
		};
...
if (mso_segment(4)=='del')
	...
	if ($id_prod=mso_segment(5))
		{
		$out .= t('Удаление товара', 'plugins/grshop').' '.$id_prod;
		mso_checkreferer();
...
foreach ($query->result_array() as $row)
	{
		...
		$tbl[$i][3]='<a href="'.$plugin_url.'/zzz1/edit/'.$row['id_prod'].'">'.$row['name_prod'].'</a>';
		$tbl[$i][4]=$row['cost_prod'];
		$tbl[$i][5]=$row['quantity_prod'];
		$tbl[$i][6]=$row['reserve_prod'];
		$tbl[$i][7]='<a href="'.$plugin_url.'/zzz1/edit/'.$row['id_prod'].'">'.t('редактировать', 'plugins/grshop').'</a>';		
		$tbl[$i][8]='<a href="'.$plugin_url.'/zzz1/del/'.$row['id_prod'].'">'.t('удалить', 'plugins/grshop').'</a>';	
	};
...
$out.=	'<h1 class="content">'.t('Товары', 'plugins/grshop').' '.$nmcat.'</h1>
	...
	form_submit('delall', t('удалить всё', 'plugins/grshop') ).$postidcat.
	form_close().
