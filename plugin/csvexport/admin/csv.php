...
*/
{
	...
	elseif ($in['f_userfile'] == '')
		{
		return 'nofile'.$in['f_userfile'];
		};
	...
	if  ($in['data_tip'] == 'prod')
		{
		$qfadd = $qff - 1; if ($in['qfadd'] > $qfadd) $qfadd = $in['qfadd'];	// это если надо больше характеристик, чем CSV-полей
		$qfadd++;
		...
		$out.= form_hidden('qfadd', $qfadd).NR;	
		};
