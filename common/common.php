...
Для вывода детей какого-то одного родителя нужно использовать примерно так:
	$catarray = get_array_db('cat');
	$slug_id = array_slug_id('cat', $catarray);	
	if (mso_segment(3)!='') $in['parent'] = $slug_id[mso_segment(3)];
*/
	{
	global $MSO;
	$out = '';
	...
	if (!isset($in['parent'])) $in['parent'] = '0'; //по умолчанию - верховный родитель (выводим всё)
	...
	if (!isset($in['headstart'])) $in['headstart']='<h2 class="box"><span>';
	if (!isset($in['headend'])) $in['headend']='</span></h2>'.NR;
	if (!isset($in['ulstart'])) $in['ulstart']='<ul class="is_link">';
	if (!isset($in['ulend'])) $in['ulend']='</ul><!-- end class="is_link" -->'.NR;
	if (!isset($in['ulchildstart'])) $in['ulchildstart']='<ul class="child">';
	if (!isset($in['ulchildend'])) $in['ulchildend']='</ul><!-- end class="child" -->'.NR;
	if (!isset($in['listart'])) $in['listart']='<li>';
	if (!isset($in['liend'])) $in['liend']='</li>'.NR;
	if (!isset($in['listartcheck'])) $in['listartcheck']='<li class="selected">';
	if (!isset($in['liendcheck'])) $in['liendcheck']='</li><!-- end class="selected" -->'.NR;
	if (!isset($in['lichildstart'])) $in['lichildstart']='<li>';
	if (!isset($in['lichildend'])) $in['lichildend']='</li>'.NR;
	if (!isset($in['lichildstartcheck'])) $in['lichildstartcheck']='<li class="selected">';
	if (!isset($in['lichildendcheck'])) $in['lichildendcheck']='</li><!-- end class="selected" -->'.NR;
	...
	if ($in['admin'] != TRUE) $CI->db->where('public_status_cat', '1');	//----если не админ - невидимые не видно
	...
	$par['i_start'] = 0;	//передаем в параметре установленный счетчик, с какой категории начинать вывод
	$par['$id_parent_cat'] = $in['parent']; //передаем параметр если нужны дети одного родителя
...
//----$out['order'] - данные заказа, $out['prod'] - массив данных товаров заказа--
	...
	$out['order'] = FALSE;
	$out['prod'] = FALSE;
	$prodinbase = 1;	//--устанавливаем признак наличия товара в БД по дефаулту
	$CI = & get_instance();
	...
	if ($query->num_rows() > 0) 
		...
		foreach ($query->result_array() as $id_prod => $prodord)
			...
			if ($prodinbase == 1)
				...
				{
				//$prod[$id_prod]['inbase'] = 0;
				$prod[$id_prod]['name_prod'] = false;
				}
			}
		}
	$out['order'] = $order;
