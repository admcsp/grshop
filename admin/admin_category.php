...
if ($post = mso_check_post(array('f_session_id', 'toadd')) or mso_segment(4)=='edit')
	...
	$public_status_cat = '';
	$menu_order_cat = '';
	$shapka=t('Новая категория товаров', 'plugins/grshop');
	if ($id_cat=mso_segment(5))
		...
		$shapka=t('Редактирование товарной категории ID', 'plugins/grshop').': '.$id_cat;
		}
	...
	<div class="block_page"><h3>'.t('Название', 'plugins/grshop').'</h3>'.form_input('name_cat', $name_cat).NR.'</div>
	<div class="block_page"><h3>'.t('Описание', 'plugins/grshop').'</h3>'.form_textarea($data = array('name'=>'descr_cat', 'value'=>$descr_cat,'rows'=>'5')).NR.'</div>
	<div class="block_page"><h3>'.t('Опубликовано', 'plugins/grshop').'</h3>'.form_checkbox('public_status_cat', '1', $checket).NR.'</div>
	<div class="block_page"><h3>'.t('Короткая ссылка', 'plugins/grshop').'</h3>'.form_input('slug_cat', $slug_cat).NR.'</div>
	<div class="block_page"><h3>'.t('Родительская категория', 'plugins/grshop').'</h3>'.form_input('parent_cat', $id_parent_cat).NR.'</div>
	<div class="block_page"><h3>'.t('Порядок в списке', 'plugins/grshop').'</h3>'.form_input('menu_order_cat', $menu_order_cat).NR.'</div>'.

	form_submit('addcat', t('Сохранить', 'plugins/grshop') ).

	'</td><td style="vertical-align: top; width: 250px;">
	<p class="info">'.
	t('Акции, действующие на категорию', 'plugins/grshop').'
	<div class="block_page"><h3>'.t('Акции', 'plugins/grshop').'</h3>'.
...
if (mso_segment(4)=='del')
	...
	if ($id_cat=mso_segment(5))
		{
		$out .= t('Удаление категории', 'plugins/grshop').' '.$id_cat;
		mso_checkreferer();
...
$tbl[1][3]=t('ссылка', 'plugins/grshop');
$tbl[1][4]=t('видимость', 'plugins/grshop');
$tbl[1][5]=t('родитель', 'plugins/grshop');
$tbl[1][6]=t('редактировать', 'plugins/grshop');
$tbl[1][7]=t('удалить', 'plugins/grshop');
...
foreach ($query->result_array() as $row)
	{
		...
		$tbl[$i][6]='<a href="'.$plugin_url.'/category/edit/'.$row['id_cat'].'">'.t('редактировать', 'plugins/grshop').'</a>';		
		$tbl[$i][7]='<a href="'.$plugin_url.'/category/del/'.$row['id_cat'].'">'.t('удалить', 'plugins/grshop').'</a>';	
	};

$out.=	'<h1 class="content">'.t('Категории товаров', 'plugins/grshop').'</h1>'.
	form_open($plugin_url .'/category').mso_form_session('f_session_id').
	form_submit('toadd', t('добавить категорию', 'plugins/grshop') ).
