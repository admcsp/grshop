...
foreach ($query->result_array() as $row)
	{
		...
		$tbl[$i][7]='<a href="'.$plugin_url.'/product/edit/'.$row['id_prod'].'">'.t('редактировать', 'plugins/grshop').'</a>';		
		$tbl[$i][8]='<a href="'.$plugin_url.'/product/del/'.$row['id_prod'].'">'.t('удалить', 'plugins/grshop').'</a>';	
	};
