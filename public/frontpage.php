...
if (is_array($arract))
	...
	$out .= '<h4>'.t('Действующие акции', 'plugins/grshop').'</h4>'.NR;
	foreach ($arract as $act)
		...
		$out .=  	'<div class="'.$stlact.'">
			<div class="'.$stlnmact.'">'.$act['name_act'].'</div><!-- class="'.$stlnmact.'" -->'.NR'
			<div class="fpdescription">'.$act['description_act'].'</div><!-- class="fpdescription" -->'.NR'
...
if ($arr_prod = get_arr_prod($par)) 	// ф-ция из биб. комм. возвр. массив товаров категории
	...
	foreach ($arr_prod as $prod)
		...
		foreach ($nf_prod as $k=>$nf)
			...
			elseif ($nf == 'cost_prod')	{
						$generalcost = $prod[$nf];
						$out .= '<h4>'.t('Цена', 'plugins/grshop').'</h4>'.NR; 
						if ($prod[$nf] != '0') $out .= '<div class="cost">'.$prod[$nf].' '.$grsh_options['money'].'</div><!-- end class="cost" -->'.NR;
						else {$out .= t('уточните у менджера', 'plugins/grshop');};
						$out .= '<br>'.NR;
						}
			elseif ($nf == 'description_prod')	
						{ if ($prod[$nf] != '') $out .= '<div class="descr">'.t('Описание:', 'plugins/grshop').' '.$prod[$nf].'</div><!-- end class="descr" -->'.NR;}
	...
	//--- порядок и оформление списка товаров тут----------------------
		...
		foreach ($arr_prod_list as $key=>$prod)
			...
			if (isset($arr_prod_list[$key]['name_cat'])) 
				{
				$name_cat = '<div class="hcat"><h2>'.$arr_prod_list[$key]['name_cat'].'</h2></div>'.NR;
			...
			if ($cp >= $pagination['$offset'] && $cp < ($pagination['$offset']+$pagination['limit']))
				...
				if ($grsh_options['echo_name_prod_list'] == '1') $out .= '<h3>'.$prod['name_prod'].'</h3>'.NR;
				if ($grsh_options['echo_articul_prod_list'] == '1') $out .= 'артикул:'.' '.$prod['articul_prod'].'<br>'.NR;
				if ($grsh_options['echo_cost_prod_list'] == '1') $out .= 'цена:'.' '.$prod['cost_prod'].'<br>'.NR;
				if ($grsh_options['echo_descr_prod_list'] == '1') $out .= 'описание:'.' '.$prod['description_prod'].'<br>'.NR;
				if ($grsh_options['echo_id_sklad_prod_list'] == '1') $out .= 'складской номер:'.$prod['id_sklad_prod'].'<br>'.NR;
